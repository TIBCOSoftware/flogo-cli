...
const (
	fileDescriptor  string = "model.json"
	fileModelGo     string = "model.go"
	fileModelTestGo string = "model_test.go"
)

func createProjectDescriptor(srcPath string, data interface{}) {

	filePath := fileDescriptor
	if len(srcPath) > 0 {
		filePath = path(srcPath, fileDescriptor)
	}
...
func createModelGoFile(srcPath string, data interface{}) {

	filePath := fileModelGo
	if len(srcPath) > 0 {
		filePath = path(srcPath, fileModelGo)
	}
...
var tplModelGoFile = `package {{.Name}}
...
func createModelTestGoFile(srcPath string, data interface{}) {

	filePath := fileModelTestGo
	if len(srcPath) > 0 {
		filePath = path(srcPath, fileModelTestGo)
	}
...
var tplModelTestGoFile = `package {{.Name}}
